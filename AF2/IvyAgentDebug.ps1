...
$jobs | ForEach-Object {
    ...
    Write-Host ""
    Write-Host "--- $($_.Name) (State: $($_.State)) ---" -ForegroundColor $(if ($_.State -eq 'Completed') { 'Green' } else { 'Yellow' })
...
if (Test-Path $langfuseDir) {
    ...
    try {
        ...
        $summaryArgs = @{ LangfuseDir = $langfuseDir; IvyDir = $ivyDir }
        ...
        Write-Host "Warning: summary.yaml generation failed: $_" -ForegroundColor Yellow
        # Write stub so Phase 3 still has some context
        @(
            "# Auto-generated by IvyAgentDebug.ps1 (partial — generation failed)"
            "sessionStatus: Unknown"
            "stopReason: `"summary generation error: $_`""
            "generationCount: 0"
            "totalInputTokens: 0"
            "totalOutputTokens: 0"
        ) | Set-Content (Join-Path $ivyDir "summary.yaml")
        Write-Host "Stub summary.yaml written." -ForegroundColor Yellow
    }
}
