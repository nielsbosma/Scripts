...
## Known Issues
- **Ivy.Tendril Plans directory deleted in merges**: RECURRING — AI agents resolving merge conflicts in Ivy-Tendril delete `src/Ivy.Tendril/Apps/Plans/` (9 files, 1115 lines). Root cause: feature branches forked from old points before Plans existed; agent uses blanket resolution that drops files not on the feature branch. Fix: restore from an earlier commit with `git checkout <hash> -- src/Ivy.Tendril/Apps/Plans/`. Added merge conflict guidelines to AGENTS.md in run 00144 to prevent future occurrences.
