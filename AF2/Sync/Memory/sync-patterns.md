...
## Known Issues
- **Ivy.Tendril Plans directory deleted in merges**: The `Apps/Plans/` directory (Models.cs, PlanYaml.cs, PlanFileExtensions.cs, ContentView.cs, SidebarView.cs, Dialogs/) is repeatedly deleted during merge conflict resolutions. This has happened at least 3 times (runs 00122-era, and run 00126 via commit 37342d50e). Fix: restore from the last commit that had the files (`git checkout <good-commit> -- src/tendril/Ivy.Tendril/Apps/Plans/`). After building Ivy.Tendril.slnx, always verify the Plans directory exists.
