...
## Known Issues
- **Ivy.Tendril Plans directory deleted in merges**: RECURRING (runs 00144, 00148) — AI agents resolving merge conflicts or doing large refactors in Ivy-Tendril delete `src/Ivy.Tendril/Apps/Plans/` (9 files, ~1126 lines). Run 00148: deleted by commit f188297 (Spectre.Console CLI conversion), not a merge conflict — the PR simply omitted the Plans files. Fix: restore from an earlier commit with `git checkout <hash>^ -- src/Ivy.Tendril/Apps/Plans/`. Added merge conflict guidelines to AGENTS.md in run 00144 but deletions still recur in non-merge PRs.
