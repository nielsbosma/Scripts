...
## Known Issues
- **Ivy.Tendril.slnx build failures**: The Ivy.Tendril project (at `Ivy-Framework/src/tendril/Ivy.Tendril/`) may fail to build on feature branches due to API incompatibilities with the current Ivy framework. Common issues include missing methods (HandleClick, Incr, HandleSubmit), missing types (Alert, ButtonType), and IVYHOOK005 analyzer violations. As of run 00057, the plan-01612-Ivy-Tendril-fresh branch has these incompatibilities. Consider skipping Tendril build or building without -warnaserror on feature branches.
