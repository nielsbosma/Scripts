...
- **TextInput labels** (`state.ToTextInput("Label").Placeholder("hint")`) — the label parameter does NOT render as visible standalone text in the DOM when `.Placeholder()` is also set. Only the placeholder text is visible inside the input. `getByText("Label")` will NOT match. Use `getByPlaceholder("hint")` to locate these inputs, or use positional selectors like `page.locator('input[type="text"]').nth(N)`.
...
- **DataTable uses Glide Data Grid (canvas-based rendering)** — Data rows are painted on an HTML5 `<canvas>` element, NOT rendered as traditional `<tr>/<td>` HTML. The `<td role="gridcell">` and `<tr role="row">` elements exist in the DOM as hidden accessibility elements but are NOT visible/clickable by Playwright. `locator.click()`, `locator.click({ force: true })`, and `page.mouse.click()` on canvas coordinates all fail to trigger `OnCellActivated`. Column headers `<th>` elements are also marked "hidden" by Playwright even when visually present. **For blade-based apps with DataTable + OnCellActivated**, detail blade interaction testing is NOT possible with standard Playwright — skip blade detail tests and verify list data loads instead. Use `page.content().includes()` for DOM content assertions instead of `toBeVisible()`.
- **DataTable column virtualization** — Columns beyond the visible viewport are NOT rendered in the DOM at all. `getByText("ColumnName")` and `page.content().includes("ColumnName")` fail for off-screen columns. Only assert on the first 2-3 visible columns. Blade layouts narrow the table to ~17% of viewport width, making most columns invisible.
